...
export const createNavigationStoreModel = navigator => {
  // Navigator for default state
  const DEFAULT_STATE = navigator.router.getStateForAction(NavigationActions.init(), null)
   ...
   */
  return NavigationEvents.named("NavigationStore")
    .props({
      /**
    ...
    .preProcessSnapshot(snapshot => {
      ...
      try {
        ...
        navigator.router.getPathAndParamsForState(snapshot.state)
        return snapshot
      } catch (e) {
